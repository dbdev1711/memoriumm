...
class AlphabetLevel extends StatelessWidget {
  ...
  List<GameConfig> _getConfigs() {
    return [
      GameConfig(mode: GameMode.alphabetRecall, rows: 3, columns: 3,
        levelTitle: language == 'cat' ? 'Fàcil (3x3)' : language == 'esp' ? 'Fácil (3x3)' : 'Easy (3x3)', requiredNumbers: 4),
      GameConfig(mode: GameMode.alphabetRecall, rows: 4, columns: 4,
        levelTitle: language == 'cat' ? 'Mitjà (4x4)' : language == 'esp' ? 'Medio (4x4)' : 'Medium (4x4)', requiredNumbers: 6),
      GameConfig(mode: GameMode.alphabetRecall, rows: 5, columns: 5,
            ...
            return Padding(
              ...
              child: ElevatedButton(
                onPressed: () => Navigator.push(context, MaterialPageRoute(
                  builder: (context) => AlphabetRecall(config: config, language: language))),
